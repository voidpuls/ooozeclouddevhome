    ...
    <div className="fixed inset-0 bg-[#0f1117] flex items-center justify-center z-50">
      <div className="text-center space-y-6 relative">
        ...
        <div className="relative mx-auto w-32 h-32">
          ...
          <div className="absolute inset-0 border-4 border-[#fbbf24] border-b-transparent rounded-full animate-spin-reverse"></div>
        </div>

        {/* Başlık ve animasyonlu noktalar */}
        <div className="space-y-4">
          <h2 className="text-[#fbbf24] text-4xl font-bold mb-2 animate-pulse">
            LIQUID
            <span className="ml-2 inline-flex space-x-1">
              {[...Array(3)].map((_, i) => (
                <span 
                  key={i}
                  className="h-1 w-1 bg-[#fbbf24] rounded-full animate-bounce"
        ...
        <div className="absolute inset-0 -z-10 opacity-20">
          {[...Array(20)].map((_, i) => (
            <div
              key={i}
              className="absolute w-1 h-1 bg-[#fbbf24] rounded-full animate-twinkle"
              style={{
                top: `${Math.random() * 100}%`,

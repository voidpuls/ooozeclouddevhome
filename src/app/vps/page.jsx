...
import { IconServer, IconHeadset, IconShieldCheck, IconRocket, IconCloudComputing, IconSettings } from "@tabler/icons-react";
import { FaDiscord } from "react-icons/fa6";
import Hero from "../../components/Hero";
import PricingTable from "../../components/PricingTable";
import Link from "../../components/Utils/Link";
import { vpsPlans } from "../../../data/plans/vps";
import { vpsFaqs } from "../../../config/faqs";
import { settings } from "../../../config/settings";
import { useTranslations } from 'next-intl';

export const metadata = {
    title: "VPS | " + settings.company_name,
    description: 'Virtual Private Servers (VPS) provide a dedicated server environment for your website or application, ensuring optimal performance and security.',
    openGraph: {
        title: "VPS | " + settings.company_name,
        description: 'Virtual Private Servers (VPS) provide a dedicated server environment for your website or application, ensuring optimal performance and security.',
    }
    ...
    return (
        <Container my="6rem">
            ...
            <Box className="service__pricing" mt="6rem" id="pricing">
                ...
                <PricingTable forceService="VPS" withLocationSwitcher withBudgetSwitcher data={vpsPlans} />
            </Box>
            ...
            <Container mt="6rem" className="service__faq">
                ...
                <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="lg">
                    <Accordion variant="separated" chevronPosition="right" multiple>
                        {vpsFaqs.slice(0, Math.ceil(vpsFaqs.length / 2)).map((item) => (
                    ...
                    <Accordion variant="separated" chevronPosition="right" multiple>
                        {vpsFaqs.slice(Math.ceil(vpsFaqs.length / 2)).map((item) => (
                            <AccordionItem key={item.value} value={item.value}>
                                <AccordionControl c="bright">{item.question}</AccordionControl>
                                <AccordionPanel>
                                    <Text c="dimmed" mt="sm">{item.answer}</Text>
